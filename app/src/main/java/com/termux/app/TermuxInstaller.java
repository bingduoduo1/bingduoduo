...
final class TermuxInstaller {
    ...
    static void setupIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        final File prefixFile = new File(TermuxService.PREFIX_PATH);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    final String stagingPrefixPath = TermuxService.FILES_PATH + "/usr-staging";
                    final File stagingPrefixFile = new File(stagingPrefixPath);
                    
                    Log.d(TAG, "run: new File Staging_prefix_path:" + stagingPrefixPath);
    ...
    private static String determineTermuxArchName() {
        ...
        throw new RuntimeException(
                "Unable to determine arch from Build.SUPPORTED_ABIS =  " + Arrays.toString(Build.SUPPORTED_ABIS));
    }
    ...
    static void deleteFolder(File fileOrDirectory) throws IOException {
        ...
        if (!fileOrDirectory.delete()) {
            throw new RuntimeException("Unable to delete " + (fileOrDirectory.isDirectory() ? "directory " : "file ")
                ...
                try {
                    ...
                    if (storageDir.exists()) {
                        try {
                            ...
                            Log.e(logTag, "Could not delete old $HOME/storage, " + e.getMessage());
                            return;
                        }
                    }
                    
                    if (!storageDir.mkdirs()) {
                        Log.e(logTag, "Unable to mkdirs() for $HOME/storage");
                        return;
                    }
                    ...
                    Log.e(logTag, "Error setting up link", e);
                }
            }
        }.start();
    }
