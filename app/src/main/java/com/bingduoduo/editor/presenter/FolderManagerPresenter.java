...
public class FolderManagerPresenter extends BasePresenter<IFolderManagerView> {
    ...
    private void getFileList(File currentFolder, String key) {
        ...
        compositeSubscription
                .add(dataManager.getFileListData(currentFolder, key).subscribe(new Subscriber<List<FileBean>>() {
                    @Override
                    public void onCompleted() {
                        compositeSubscription.remove(this);// 任务完成
    ...
    /*
     * editMode 0：无 1：编辑模式 2：复制粘贴模式 3：剪切粘贴模式 4：删除
     */
    ...
    public void paste() {
        ...
        if (meditmode == EDIT_MODE_CUT_PARSE) {
            // 剪切粘贴
            // 判断是否要剪切的文件夹是否包含在路径
            String path = currentPath();
            for (FileBean bean : temp) {
            ...
            compositeSubscription.add(dataManager.cutFile(temp, path).subscribe(new Subscriber<FileBean>() {
                @Override
                public void onCompleted() {
                    compositeSubscription.remove(this);// 任务完成
                ...
                @Override
                public void onNext(FileBean file) {
                    // 粘贴成功
                    getMvpView().addFilePosition(0, file);
                }
            }));
        } else if (meditmode == EDIT_MODE_COPY_PARSE) {
            // 复制粘贴
            String path = currentPath();
            for (FileBean bean : temp) {
            ...
            compositeSubscription.add(dataManager.copyFile(temp, path).subscribe(new Subscriber<FileBean>() {
                @Override
                public void onCompleted() {
                    compositeSubscription.remove(this);// 任务完成
                    callHideProgress(IFolderManagerView.CALL_COPY_PASTE);
