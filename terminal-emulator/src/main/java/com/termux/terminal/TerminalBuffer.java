...
public final class TerminalBuffer {
    ...
    public String getTranscriptText() {
        return getSelectedText(0, -getActiveTranscriptRows(), mcolumns, screenRows).trim();
    }
    ...
    public String getSelectedText(int selX1, int selY1, int selX2, int selY2) {
        ...
        for (int row = selY1; row <= selY2; row++) {
            ...
            if (!rowLineWrap && row < selY2 && row < screenRows - 1)
            {
                builder.append('\n');
            }
        }
    ...
    public int getActiveRows() {
        return mactivetranscriptrows + screenRows;
    }
     ...
     *
     * <pre>
     * - External coordinate system: -mactivetranscriptrows to screenRows-1, with the screen being 0..screenRows-1.
     * - Internal coordinate system: the screenRows lines starting at mscreenfirstrow comprise the screen, while the
     ...
     * [ ...                            ]     [ ...                                     ]
     * [ screenRows-1                  ]     [ mscreenfirstrow + screenRows-1         ]
     * </pre>
     *
    ...
    public int externalToInternalRow(int externalRow) {
        if (externalRow < -mactivetranscriptrows || externalRow > screenRows)
        {
            throw new IllegalArgumentException("extRow=" + externalRow + ", screenRows=" + screenRows
        ...
        if (newColumns == mcolumns && newRows <= mtotalrows) {
            ...
            if (shiftDownOfTopRow > 0 && shiftDownOfTopRow < screenRows) {
                ...
                if (shiftDownOfTopRow != actualShift) {
                    ...
                    {
                        allocateFullLineIfNecessary((mscreenfirstrow + screenRows + i) % mtotalrows)
                            .clear(currentStyle);
                    }
            ...
            for (int externalOldRow = -oldActiveTranscriptRows; externalOldRow < oldScreenRows; externalOldRow++) {
                ...
                internalOldRow =
                        (internalOldRow < 0) ? (oldTotalRows + internalOldRow) : (internalOldRow % oldTotalRows);
    ...
    public void scrollDownOneLine(int topMargin, int bottomMargin, long style) {
        if (topMargin > bottomMargin - 1 || topMargin < 0 || bottomMargin > screenRows)
        {
            throw new IllegalArgumentException(
                "topMargin=" + topMargin + ", bottomMargin=" + bottomMargin + ", screenRows=" + screenRows);
        }
        ...
        if (mactivetranscriptrows < mtotalrows - screenRows)
        {
    ...
    public void blockCopy(int sx, int sy, int w, int h, int dx, int dy) {
        ...
        if (sx < 0 || sx + w > mcolumns || sy < 0 || sy + h > screenRows || dx < 0 || dx + w > mcolumns || dy < 0
                || dy + h > screenRows)
        {
            throw new IllegalArgumentException();
        }
    ...
    public void blockSet(int sx, int sy, int w, int h, int val, long style) {
        if (sx < 0 || sx + w > mcolumns || sy < 0 || sy + h > screenRows) {
            throw new IllegalArgumentException("Illegal arguments! blockSet(" + sx + ", " + sy + ", " + w + ", " + h
                    + ", " + val + ", " + mcolumns + ", " + screenRows + ")");
        }
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
    ...
    public void setChar(int column, int row, int codePoint, long style) {
        if (row >= screenRows || column >= mcolumns)
        {
            throw new IllegalArgumentException(
                "row=" + row + ", column=" + column + ", screenRows=" + screenRows + ", mcolumns=" + mcolumns);
        }
    ...
    public void setOrClearEffect(int bits, boolean setOrClear, boolean reverse, boolean rectangular, int leftMargin,
            int rightMargin, int top, int left, int bottom, int right) {
